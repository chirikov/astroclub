...
$res1 = @mysql_query("select realdelta from astro where day = ".$x) or ferror(2);
$delta = @mysql_result($res1, 0, 'realdelta') or ferror(3);
...
if(window.event.x == document.getElementById('dix1').value && window.event.y == document.getElementById('dix2').value) {
	...
	while(n != xx.length) {
		if(x <= xx[n]+3 && x >= xx[n]-3 && y <= yy[n]+3 && y >= yy[n]-3) {
			var ss = ngc[n].search(',');
			var urls;
			if(ss != -1) {urls = ngc[n].substr(0, ss);} else {urls = ngc[n];}
			urls = urls.replace(' ', '@');
...
//////////
parent.document.getElementById('cursor').innerHTML = '<table class=text><tr><td align=right valign=top>Позиция курсора:&nbsp;</td><td>Азимут: '+azim+'&deg;, Высота: '+heig+'&deg;<br>Прямое восхождение: '+Math.round(ra*10)/10+' ч., Склонение: '+Math.round(180*dec/3.14159)+'&deg;</td></tr></table>';
"
>
