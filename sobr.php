...
function listpage($page) {
	function pages($page) {
		$q = @mysql_query("select id from sobr where 1") or ferror(2);
		$n = @mysql_num_rows($q) or ferror(3);
		$pages = ceil($n/getconf("sobr_per_page"));
		...
		for($i=1;$i<=$pages;$i++) {
			...
			else print "<a href='sobr.php?page=".$i."'>$i</a>";
		}
		print "</div>";
	}
	
	$q00 = mysql_query("select id from sobr where 1");
	if(mysql_num_rows($q00) == 0) {
		print "<br>Собраний в базе данных нет.";
	}
	else {
		pages($page);
		print "<br><br>";
		$q = mysql_query("select id, date, name from sobr where 1 order by date desc limit ". ($page-1)*getconf("sobr_per_page") .", ".getconf("sobr_per_page"));
		print "<table class=text align=center cellspacing=0 width='100%'>";
		$q2 = mysql_query("select id from sobr where date > ".time());
		if(mysql_num_rows($q2)>0) print "<tr><td colspan=2>Планируются собрания: <br><br></td></tr>";
		$printed_theprev = 0;
		while($row = mysql_fetch_assoc($q)) {
			if($row['date']>time()) print "<tr><td bgcolor='#B4D5FA'>".date('d.m.Y, H:i', $row['date'])."</td><td width='100%' style='border : 1px solid #B4D5FA;'>".$row['name']."</td></tr>";
				...
				if($printed_theprev==0) {
					print "<tr><td colspan=2><br><br>Прошедшие собрания: <br><br></td></tr>";
					$printed_theprev = 1;
				}
				print "<tr><td bgcolor='#EBE9DA'>".date('d.m.Y, H:i', $row['date'])."</td><td width='100%' style='border : 1px solid #EBE9DA;'><a href='sobr.php?act=read_info&sid=".$row['id']."'>".$row['name']."</a></td></tr>";
			}
		}
		print "</table>";
		print "<br><br>";
...
function read_info($sid) {
	$q = mysql_query("select * from sobr where id = ".$sid);
...
switch ($act) {
	default: $heading = "Собрания астроклуба"; break;
	case "read_info": $heading = "Подробности собрания"; break;
}
...
<TABLE WIDTH=500 BORDER=0 CELLPADDING=0 CELLSPACING=0 align="center">
	...
	if(!isset($sid)) {
		$qq = @mysql_query("select id from sobr where 1 order by date desc limit 0, 1");
